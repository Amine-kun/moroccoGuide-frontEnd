...
class Hotels extends React.Component {
	...
	render(){
			const {LikeHandler, GetArticle, article,showArticle, hideArticle, isSigned} = this.props;
			...
			const post = namearr.map((imagename) =>(
				<div className="br2 ba dark-gray b--black-10   w-100 w-50-m w-25-l mw5 shadow" key={imagename["photo"]}>
					  <img src={`http://localhost:3000/images/${imagename["photo"]}`} onClick={()=>GetArticle(imagename)} className="db pointer center w-100 br2 fknimg2 br--top" alt={imagename["photo"]}/>
					  <div className="pa2 ph3-ns pb3-ns bg-white">
					    ...
					    <p className="f6 lh-copy measure content mt2 mid-gray">
					      {imagename["desc"]}
					    </p>
					...
					<div className={ !showArticle ? `hidden` : `shown`}>
					    ...
					    { isSigned 
        					...
        					control={<Checkbox icon={<FavoriteBorder />} 
                  			checkedIcon={<Favorite />}
         					 name="checkedH" />}    
         					 label=<b>{article["likes"]}</b> className="react"
